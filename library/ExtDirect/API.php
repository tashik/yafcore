...
class ExtDirect_API {
    ...
    public function getAPI() {
        ...
        foreach($this->_classes as $class => $settings) {
            ...
            if(class_exists($settings['prefix'] . $class . $settings['suffix'])) {
                $rClass = new ReflectionClass($settings['prefix'] . $class . $settings['suffix']);
                ...
                foreach($rMethods as $rMethod) {
                    if(
                        ...
                        if($isRemote) {
                            ...
                            if (!empty($settings['methodSuffix'])) {
                              $method['name'] = preg_replace("/^(.+){$settings['methodSuffix']}$/", '\1', $method['name']);
                            }
                            //$method['name'] = strtolower(preg_replace("/([a-z])([A-Z])/", '\1_\2', $method['name']));
